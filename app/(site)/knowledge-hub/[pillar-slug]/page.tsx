...
export async function generateMetadata(props: PillarPageProps): Promise<Metadata> {
  ...
  return {
    title: `${pillar.title} | Knowledge Hub | Honworth`,
  ...
  return (
    <div className="bg-ivory min-h-screen">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-16">
        ...
        {/* Pillar Header */}
        <header className="mb-12 border-b border-sage/30 pb-8">
          <span className="text-xs font-semibold tracking-[0.2em] uppercase text-gold block mb-3">
            Knowledge Pillar
          </span>
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-serif text-deep-green leading-tight mb-4">
            {pillar.title}
          </h1>
          <p className="text-lg font-sans text-charcoal/80 leading-relaxed max-w-2xl">
            {pillar.description}
          </p>
        </header>

        {/* Sub-topics Outline Section */}
        <section aria-labelledby="subtopics-heading" className="space-y-6">
          <div className="flex items-center justify-between mb-6">
            <h2 id="subtopics-heading" className="font-serif text-2xl text-deep-green">
              Curriculum & Topic Outline
            </h2>
            <span className="text-xs font-sans text-charcoal/60 bg-sage-mist/40 px-3 py-1 rounded-full border border-sage/20">
              {pillar.subtopics.length} Topics
            </span>
          </div>

          <div className="grid gap-4 sm:gap-5">
            {pillar.subtopics.map((topic, index) => (
              <div
                key={index}
                className="bg-white/60 border border-sage/30 rounded-md p-6 sm:p-7 hover:border-sage transition-all duration-200"
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <h3 className="font-serif text-xl text-deep-green mb-2 leading-snug">
                      {topic.title}
                    </h3>
